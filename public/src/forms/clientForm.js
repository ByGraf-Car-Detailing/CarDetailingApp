...
function checkNameFields() {
  ...
  if (firstName.length >= 2 && lastName.length >= 2) {
    ...
    // Mostra subito Email (Ditta e opzionale, default NO)
...
// === CAP: Filtro solo numeri + scroll a Citta + checkAddress ===
capField.addEventListener("input", (e) => {
  e.target.value = e.target.value.replace(/[^0-9]/g, "");
  // Scroll a Citta quando CAP e valido
...
// capField listener gia definito sopra con filtro numerico
...
export async function handleClientFormSubmit(formNode, quickMode = false, options = {}) {
  try {
    ...
    if (!type) return { error: "Errore: Seleziona il tipo cliente.", field: "#clientType" };
    if (type === "person" && !data.firstName) return { error: "Errore: Inserisci il nome.", field: "#firstName" };
    if (type === "person" && !data.lastName) return { error: "Errore: Inserisci il cognome.", field: "#lastName" };
    if (type === "company" && !data.companyName) return { error: "Errore: Inserisci ragione sociale.", field: "#companyName" };
    if (!email) return { error: "Errore: Inserisci email.", field: "#email" };
    if (!phonePrefix) return { error: "Errore: Seleziona il prefisso telefonico.", field: "#phonePrefix" };
    if (!phoneNumber) return { error: "Errore: Inserisci il numero di telefono.", field: "#phoneNumber" };
    if (!address.street) return { error: "Errore: Inserisci la via.", field: "#street" };
    if (!address.number) return { error: "Errore: Inserisci il numero civico.", field: "#streetNumber" };
    if (!address.cap) return { error: "Errore: Inserisci il CAP.", field: "#cap" };
    if (!address.city) return { error: "Errore: Inserisci la citta.", field: "#city" };
    
    // P.IVA obbligatoria per Azienda
    if (type === "company" && !data.fiscalCode) return { error: "Errore: Inserisci P.IVA / Codice Fiscale per l'azienda.", field: "#fiscalCode" };

    // Validazione asincrona azienda
    if (type === "person" && isContact && companyId) {
      const ref = doc(db, "clients", companyId);
      const snap = await getDoc(ref);
      if (!snap.exists()) return { error: "Errore: Azienda selezionata non trovata.", field: "#linkedCompanyId" };
    }
    ...
    if (!emailClients.empty) return { error: "Errore: Email gia esistente.", field: "#email" };
    if (!phoneClients.empty) return { error: "Errore: Telefono gia registrato.", field: "#phoneNumber" };
    ...
    if (type === "person") {
      ...
      for (const docu of allClients.docs) {
        ...
        if (
          ...
          if (c.phone === phone) {
            return { error: "Errore: Esiste gia un cliente con stesso nome e telefono.", field: "#phoneNumber" };
          }
          if (
            ...
            return { error: "Errore: Esiste gia un cliente con stesso nome e indirizzo.", field: "#street" };
          }
        }
      }
    }
    ...
    if (quickMode) {
      ...
      msgBox.textContent = "OK: Cliente salvato con successo.";
      resetClientForm();
    }
  } catch (err) {
    return { error: "Errore: Errore: " + (err.message || "sconosciuto") };
  }
}
