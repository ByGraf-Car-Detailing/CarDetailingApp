...
export async function loadClientForEdit(clientId) {
  ...
  editForm.onsubmit = async (e) => {
    e.preventDefault();
    if (disabled) {
      alert("Cliente disattivato. Non e possibile salvare.");
      return;
    }
    ...
    if (isPerson) {
      ...
      if (isContactChecked && selectedCompany === "NO") {
        alert("Devi selezionare un'azienda se il cliente e un contatto aziendale.");
        return;
      }
    }
    ...
    try {
      ...
      alert("Errore: Salvataggio non riuscito.");
    }
  };
