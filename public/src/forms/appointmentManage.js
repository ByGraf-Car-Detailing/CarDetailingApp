  ...
  function renderList(appointments) {
    ...
    appointments.forEach(d => {
      ...
      if ((userRole === "admin") || (userRole === "staff" && d.createdBy === userEmail)) {
        actions += `<button class="btn btn--icon btn--ghost editBtn" data-id="${d.id}">Modifica</button>`;
      }
      if (userRole === "admin") {
        actions += `<button class="btn btn--icon btn--danger deleteBtn" data-id="${d.id}">Elimina</button>`;
      }
    ...
    if (editForm) {
      editForm.onsubmit = async (e) => {
        ...
        if (!startReception || !endReception || !startWork || !endWork || !startDelivery || !endDelivery) {
          msgBox.textContent = "Errore: Compila tutte le date/ore.";
          return;
        }
        ...
        if (
          ...
          msgBox.textContent = "Errore: Ordine delle date/ore non valido.";
          return;
        }

        if (((new Date(endWork) - new Date(startWork)) / (1000 * 60 * 60)) < 2) {
          msgBox.textContent = "Errore: Durata lavorazione minima 2h.";
          return;
        }
