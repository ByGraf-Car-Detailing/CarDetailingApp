...
function renderStepLocation() {
  stepLocation.innerHTML = `
    ...
    <select id="locationSelectAppointment" required>
      ...
      <option value="Cadenazzo">Cadenazzo</option>
    </select>
  `;
...
async function renderStepOperator() {
  ...
  snap.forEach(docSnap => {
    ...
    opt.textContent = d.displayName || docSnap.id;
...
function openQuickClientModal(onSuccessCallback) {
    ...
    import('./clientForm.js').then(module => {
      ...
      // Filtro CAP numerico + scroll a Citta
      if (capField) {
...
async function renderStepJobType() {
  stepJobType.innerHTML = `
    ...
    <input type="number" id="priceInputAppointment" min="0" placeholder="Prezzo (EUR)" style="margin-left:10px; width:110px;" />
  `;
  ...
  snap.forEach(docSnap => {
    ...
    const opt = document.createElement("option");
    opt.value = docSnap.id;
    opt.textContent = `${d.description} (${normalizedDefaultPrice} EUR)`;
...
form.addEventListener("submit", async (e) => {
  ...
  if (
    ...
    msgBox.textContent = "Errore: Compila tutti i campi obbligatori.";
    return;
  }
  ...
  try {
    ...
    msgBox.textContent = "Errore: Salvataggio non riuscito. Riprova.";
  }
});
