...
Assert-Contains -Path "AGENTS.md" -Pattern "codex_session_id == claude_session_id == CODEX_THREAD_ID" -Label "AGENTS same-id binding"
Assert-Contains -Path "AGENTS.md" -Pattern "SESSION_ID_SOURCE_MISSING" -Label "AGENTS hard-stop source missing"
...
Assert-Contains -Path ".github/pull_request_template.md" -Pattern "binding_source: CODEX_THREAD_ID" -Label "PR template binding source field"
...
if (-not (Test-Path "scripts/validate-coworking-trace.mjs")) {
  Write-Error "FAILED [coworking trace validator] -> scripts/validate-coworking-trace.mjs is missing"
  exit 1
}
Write-Host "PASS   [coworking trace validator]"
